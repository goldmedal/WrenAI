...
Public-zone steps never receive the prepared semantic snapshot. They receive
the **capability card** instead: schema only, generated by code from the bound
project. The `wren-context-loader` generator writes a capability catalog
(`--catalog-out`) next to the prepared document — names, types, descriptions,
primary keys, relationships, cube members with declared `granularities` and
`date_range`, view names, and enum meanings from column `enum_values` or
`[enum]`-tagged descriptions — and the harness renders it through a second
allow-list, so a new MDL field never leaks by default and no `ref_sql`, view
...
which host context it would receive (card or snapshot, with the card's digest),
