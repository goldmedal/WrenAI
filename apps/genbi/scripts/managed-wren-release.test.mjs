...
function fixture() {
  ...
  const mirror = "https://github.com/goldmedal/WrenAI/releases/download/managed-wren-fixture/";
...
test("fork inventory rejects upstream and other repository mirrors before fetching", async (t) => {
  const root = await temp(t);
  for (const repository of ["Canner/WrenAI", "other/WrenAI"]) {
    const { candidate, inputs } = fixture();
    candidate.python.mirror.url = candidate.python.mirror.url.replace("goldmedal/WrenAI", repository);
    candidate.wheels[0].url = candidate.wheels[0].url.replace("goldmedal/WrenAI", repository);
    await assert.rejects(refetchExactPublishAssets(candidate, inputs, path.join(root, "assets"), () => assert.fail("must not fetch")));
  }
  assert.deepEqual(await readdir(root), []);
});

test("actual publication commands target only the fork even with an upstream ambient GH_REPO", async (t) => {
  const root = await temp(t); const bin = path.join(root, "bin");
  ...
  const guards = workflow.split("\n").filter((line) => line.includes('test "$GITHUB_REPOSITORY"'));
  assert.equal(guards.length, 2);
  const publish = workflow.split("\n").find((line) => line.trimStart().startsWith("gh release create"));
  assert.ok(publish);
  for (const guard of guards) {
    const run = (repository) => spawnSync("/bin/sh", ["-ec", guard + "\n" + publish], {
      cwd: root, env: { PATH: bin, CAPTURE: capture, GH_REPO: "Canner/WrenAI", GITHUB_REPOSITORY: repository, RUNTIME_TAG: "managed-wren-fixture", GITHUB_SHA: "a".repeat(40) },
    });
    assert.notEqual(run("Canner/WrenAI").status, 0);
    assert.deepEqual(await readdir(bin), ["gh"]);
    await assert.rejects(readFile(capture), { code: "ENOENT" });
    assert.equal(run("goldmedal/WrenAI").status, 0);
    const args = (await readFile(capture, "utf8")).trim().split("\n");
    assert.equal(args[args.indexOf("--repo") + 1], "goldmedal/WrenAI");
